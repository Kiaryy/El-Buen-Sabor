            ...
            if (entity === "proveedores" || entity === "insumos" || entity==="bebidas") {
                ...
                tr.innerHTML = config.columns.map(col => {
                    ...
                    if (col === "pedido") {
                        ...
                        // Combinar los nombres de platos y bebidas en una sola cadena
                   
                        const listItem = [
                            
                            ...item.pedido.platos.map(plate => plate.plateName),
                            ...item.pedido.bebidas.map(drink => drink.plateName)
                        ].join(', ')
                    
                        return `<td>${listItem !== '' ? listItem : 'falta'}</td>`;
                    }else{
                        return `<td>${item[col] !== undefined ? item[col] : 'falta'}</td>`;
                    }
                }).join  ('');
